		...
		cam.Start3D2D(nearest, ang, 0.1)
			...
			if self:GetMaxRepairs() > 0 or self:GetMaxNailHealth() > 0 then
				...
				if displayowner then
					...
					draw.SimpleText(displayowner, "ZS3D2DFont2Smaller", 0, y + 20, col, TEXT_ALIGN_CENTER)
					draw.SimpleText(math.floor(nhp) .. "/" .. math.floor(self:GetMaxNailHealth()), "ZS3D2DFont2Smaller", x + 60, y - 45, col, TEXT_ALIGN_CENTER)
				end
		cam.End3D2D()
