...
local panim = SimplePanel(ptool)
	...
	local agrid = vgui.Create( "DGrid", panim )
		...
		local animations = {
			...
			{ "Draw", ACT_VM_DRAW, nil },
			{ "Holster", ACT_VM_HOLSTER, nil },
			{ "Throw grenade", ACT_VM_THROW, PLAYER_ATTACK1 },
			{ "Idle", ACT_VM_IDLE, nil },
			
			{ "(Fists) Draw", "fists_draw", nil },
			{ "(Fists) Left Punch", "fists_left", nil },
			{ "(Fists) Right Punch", "fists_right", nil },
			{ "(Fists) Uppercut", "fists_uppercut", nil },
			{ "(Fists) Idle 1", "fists_idle_01", nil },
			{ "(Fists) Idle 2", "fists_idle_02", nil },
		}
		...
		for k, anim in pairs( animations ) do
			...
			local abtn = vgui.Create( "DButton", agrid )
				...
				abtn.DoClick = function()
					...
					if (anim[2] != nil) then
						if type( anim[2] ) == "string" then
							local seq = vm:LookupSequence( anim[2] )
							if seq then
								vm:SendViewModelMatchingSequence( seq )
							end
						else
							vm:SendViewModelMatchingSequence( vm:SelectWeightedSequence( anim[2] ) )
						end
					end
					
					-- boo, I dont need this anymore
